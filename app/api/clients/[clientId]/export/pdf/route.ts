// GET /api/clients/[clientId]/export/pdf
// Downloads a PDF summary with key metrics and a transactions table.
...
import { buildClientPdf } from "@/lib/exports/pdf";
...
export async function GET(
  ...
  try {
    ...
    const buffer = buildClientPdf(data);
    await auditClientExport(user, data.client, "pdf");

    const slug = sanitizeFilename(data.client.client_name);
    const filename = `${slug || clientId}-report.pdf`;
    ...
    console.error("[api/clients/export/pdf] failed:", err);
